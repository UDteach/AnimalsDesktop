...
preview exposes 56 selectable animal sprites that walk near the Windows taskbar
or along the bottom edge above the Mac Dock.

Public page: <https://udteach.github.io/AnimalsDesktop/>

Current app version: `v0.2.10`

## Current Status

`v0.2.10` is an early public test release for 56 accepted 62-frame ImageGen
motion animals. It builds on the remaining GitHub Pages candidate wave from
v0.2.9, adds two color-tone chinchilla variants, replaces the black-eyed white
chipmunk slot with the red-eyed no-marking true albino chipmunk, and refreshes
the blue hooded fancy rat toward a cooler gray-blue hood. The release keeps the
original sixteen preview animals and the latest accepted asset lanes:
...
The v0.2.10 preview intentionally exposes only the 56 accepted runtime animals
listed above. The black-eyed white chipmunk source remains cataloged as legacy
evidence, but the public runtime uses `true_albino_chipmunk` for the albino
chipmunk slot. Unverified candidate species should not appear in
...
animal before production starts. After the v0.2.10 release, the visible Pages
candidate queue is empty; the next repair priority is runtime-visible albino
chipmunk readability if the current source still needs a stronger ImageGen pass.
...
animals plus Mac distribution. `v0.2.10` is an explicit test-preview exception
for the 56-animal runtime roster before the full 10-set motion gate.
...
go run github.com/tc-hib/go-winres@v0.3.1 make --arch amd64 --out cmd/animalsdesktop/rsrc --file-version v0.2.10 --product-version v0.2.10
...
$env:GOOS = "windows"
$env:GOARCH = "amd64"
go build -buildvcs=false -ldflags="-H=windowsgui -s -w -X main.appVersion=v0.2.10" -o dist\AnimalsDesktop.exe ./cmd/animalsdesktop
...
```bash
VERSION=v0.2.10 GOARCH=arm64 scripts/build_macos.sh
VERSION=v0.2.10 GOARCH=amd64 scripts/build_macos.sh
```
