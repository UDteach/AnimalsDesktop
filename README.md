...
Current app version: `v0.2.14`

## Current Status

`v0.2.14` is an early public test release for 57 accepted 62-frame ImageGen
...
The v0.2.14 preview intentionally exposes only the 57 accepted runtime animals
...
animal before production starts. After the v0.2.14 release, the visible Pages
...
animals plus Mac distribution. `v0.2.14` is an explicit test-preview exception
...
go run github.com/tc-hib/go-winres@v0.3.1 make --arch amd64 --out cmd/animalsdesktop/rsrc --file-version v0.2.14 --product-version v0.2.14
...
$env:GOOS = "windows"
$env:GOARCH = "amd64"
go build -buildvcs=false -ldflags="-H=windowsgui -s -w -X main.appVersion=v0.2.14" -o dist\AnimalsDesktop.exe ./cmd/animalsdesktop
...
```bash
VERSION=v0.2.14 GOARCH=arm64 scripts/build_macos.sh
VERSION=v0.2.14 GOARCH=amd64 scripts/build_macos.sh
```
