...
preview exposes 57 selectable animal sprites that walk near the Windows taskbar
or along the bottom edge above the Mac Dock.

Public page: <https://udteach.github.io/AnimalsDesktop/>

Current app version: `v0.2.13`

## Current Status

`v0.2.13` is an early public test release for 57 accepted 62-frame ImageGen
motion animals. It keeps the v0.2.12 roster and fixes settings selection drift
by saving stable animal IDs alongside the older numeric picker index. It also
groups the animal pickers by broad animal type on Windows and macOS. The release
keeps the original sixteen preview animals and the latest accepted asset lanes:
...
The v0.2.13 preview intentionally exposes only the 57 accepted runtime animals
listed above. The black-eyed white chipmunk source remains cataloged as source
evidence, while `true_albino_chipmunk` is now backed by the new ImageGen-only
Direction B source. Unverified candidate species should not appear in the
...
animal before production starts. After the v0.2.13 release, the visible Pages
candidate queue is empty; the next asset priorities should be added explicitly
...
animals plus Mac distribution. `v0.2.13` is an explicit test-preview exception
for the 57-animal runtime roster before the full 10-set motion gate.
...
go run github.com/tc-hib/go-winres@v0.3.1 make --arch amd64 --out cmd/animalsdesktop/rsrc --file-version v0.2.13 --product-version v0.2.13
...
$env:GOOS = "windows"
$env:GOARCH = "amd64"
go build -buildvcs=false -ldflags="-H=windowsgui -s -w -X main.appVersion=v0.2.13" -o dist\AnimalsDesktop.exe ./cmd/animalsdesktop
...
```bash
VERSION=v0.2.13 GOARCH=arm64 scripts/build_macos.sh
VERSION=v0.2.13 GOARCH=amd64 scripts/build_macos.sh
```
