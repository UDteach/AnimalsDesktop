...
TARGET_MAX_H = 34
BASELINE_Y = 57
RAW_NAME = "frame-43-attempt-03.png"
...
def main() -> None:
    ...
    frame_path = FRAMES / "frame-43.png"
    ...
    if frame.size != (96, 64):
        raise ValueError(f"frame-43 has wrong size {frame.size}")
    if stats["visible_pixels"] <= 0:
        raise ValueError("frame-43 has empty alpha")
    if stats["edge_alpha_pixels"] != 0:
        raise ValueError(f"frame-43 has non-zero edge alpha: {stats['edge_alpha_pixels']}")
    if stats["transparent_residual_magenta_pixels"] or stats["transparent_white_gray_pixels"]:
        raise ValueError("frame-43 has residual background pixels")
    frame.save(frame_path)

    write_contact(list(range(0, 45)), "lovebird-peach-faced-00-44")
    write_contact(list(range(37, 45)), "lovebird-peach-faced-37-44")
    ...
    (QA / "prepare-report-frame-43-attempt-03.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
