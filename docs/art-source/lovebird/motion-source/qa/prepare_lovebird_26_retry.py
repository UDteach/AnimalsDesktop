...
TARGET_MAX_H = 42
BASELINE_Y = 57
RAW_NAME = "frame-26-attempt-03.png"
...
def main() -> None:
    ...
    for idx in list(range(26)) + [27, 28]:
        if not (FRAMES / f"frame-{idx:02d}.png").exists():
            raise FileNotFoundError(f"existing frame-{idx:02d}.png is missing")

    raw_path = RAW / RAW_NAME
    alpha_path = ALPHA / RAW_NAME
    frame_path = FRAMES / "frame-26.png"
    ...
    if frame.size != (96, 64):
        raise ValueError(f"frame-26 has wrong size {frame.size}")
    if stats["visible_pixels"] <= 0:
        raise ValueError("frame-26 has empty alpha")
    if stats["edge_alpha_pixels"] != 0:
        raise ValueError(f"frame-26 has non-zero edge alpha: {stats['edge_alpha_pixels']}")
    if stats["transparent_residual_magenta_pixels"] or stats["transparent_white_gray_pixels"]:
        raise ValueError("frame-26 has residual background pixels")
    frame.save(frame_path)

    write_contact(list(range(0, 29)), "lovebird-peach-faced-00-28")
    write_contact(list(range(21, 29)), "lovebird-peach-faced-21-28")
    ...
    (QA / "prepare-report-frame-26-attempt-03.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
